...
static const char PEER_ID[] = "pZ";
static const char RESUME_FOLDER[] = "PZ_backup";
static const char USER_AGENT[] = "pzlauncher/" PZL_VERSION_2;
...
namespace
{
    ...
    bool loadTorrentResumeData(const QByteArray &data, BitTorrent::CreateTorrentParams &torrentParams, int &prio, BitTorrent::MagnetUri &magnetUri);
...
Session::Session(QObject *parent)
    ...
    , m_asyncIOThreads(4)
    , m_checkingMemUsage(16)
    , m_diskCacheSize(64)
    , m_diskCacheTTL(60)
    , m_useOSCache(true)
    , m_guidedReadCacheEnabled(true)
#ifdef Q_OS_WIN
    , m_coalesceReadWriteEnabled(true)
#else
    ...
    , m_maxActiveDownloads(10)
    , m_maxActiveUploads(10)
    , m_maxActiveTorrents(10)
    ...
    , m_torrentExportDirectory(Config::TMP_PATH_ROOT+"/export/")
    , m_finishedTorrentExportDirectory(Config::TMP_PATH_ROOT+"/finishedExport/")
    , m_globalDownloadSpeedLimit(0)
    , m_globalUploadSpeedLimit(0)
    , m_altGlobalDownloadSpeedLimit(1024*100)
    , m_altGlobalUploadSpeedLimit(1024*7)
    , m_isAltGlobalSpeedLimitEnabled(false)
    , m_isBandwidthSchedulerEnabled(false)
    , m_saveResumeDataInterval(60)
    , m_port(Config::TORRENT_PORT)
    ...
    , m_defaultSavePath(Config::TMP_PATH_ROOT + "/defSave/")
    , m_tempPath(Config::TMP_PATH_ROOT + "/temp/")
...
{
    m_storedCategories = QVariantMap();
    m_storedCategories.insert("def",Config::TMP_PATH_ROOT + "/storage/");
    ...
    const std::string peerId = libt::generate_fingerprint(PEER_ID, PZL_VERSION_MAJOR, PZL_VERSION_MINOR, PZL_VERSION_BUGFIX, PZL_VERSION_BUILD);
    ...
    m_timerUpdateTorrentStatus = new QTimer(this);
    m_timerUpdateTorrentStatus->setSingleShot(true);
    //m_timerUpdateTorrentStatus->setInterval(100);
    connect(m_timerUpdateTorrentStatus, &QTimer::timeout, this, [this]() { qWarning("timeout"); });
    m_timerUpdateTorrentStatus->start(1000);
...
void Session::setTempPathEnabled(bool enabled)
{
    if (enabled != isTempPathEnabled()) {
        ...
        for (BitTorrent::TorrentHandle *const torrent : asConst(m_torrents))
...
QString Session::torrentTempPath(const BitTorrent::TorrentInfo &torrentInfo) const
{
...
void Session::applyBandwidthLimits(libtorrent::settings_pack &settingsPack)
{
    ...
    settingsPack.set_int(libt::settings_pack::download_rate_limit, globalDownloadSpeedLimit());
    settingsPack.set_int(libt::settings_pack::upload_rate_limit, globalUploadSpeedLimit());
}
...
void Session::enableTracker(bool enable)
{
    ...
    if (enable) {
        if (!m_tracker)
            m_tracker = new BitTorrent::Tracker(this);
...
void Session::processShareLimits()
{
    ...
    for (BitTorrent::TorrentHandle *const torrent : asConst(torrents())) {
        if (torrent->isSeed() && !torrent->isForced()) {
            if (torrent->ratioLimit() != BitTorrent::TorrentHandle::NO_RATIO_LIMIT) {
                const qreal ratio = torrent->realRatio();
                qreal ratioLimit = torrent->ratioLimit();
                if (ratioLimit == BitTorrent::TorrentHandle::USE_GLOBAL_RATIO)
                ...
                if (ratioLimit >= 0) {
                    ...
                    if ((ratio <= BitTorrent::TorrentHandle::MAX_RATIO) && (ratio >= ratioLimit)) {
            ...
            if (torrent->seedingTimeLimit() != BitTorrent::TorrentHandle::NO_SEEDING_TIME_LIMIT) {
                const int seedingTimeInMinutes = torrent->seedingTime() / 60;
                int seedingTimeLimit = torrent->seedingTimeLimit();
                if (seedingTimeLimit == BitTorrent::TorrentHandle::USE_GLOBAL_SEEDING_TIME)
                ...
                if (seedingTimeLimit >= 0) {
                    ...
                    if ((seedingTimeInMinutes <= BitTorrent::TorrentHandle::MAX_SEEDING_TIME) && (seedingTimeInMinutes >= seedingTimeLimit)) {
...
{
    addTorrent_impl(BitTorrent::CreateTorrentParams(m_downloadedTorrents.take(url)), BitTorrent::MagnetUri(magnetUri));
}
...
void Session::handleDownloadFinished(const QString &url, const QByteArray &data)
{
    ...
    addTorrent_impl(BitTorrent::CreateTorrentParams(m_downloadedTorrents.take(url))
                    , BitTorrent::MagnetUri(), BitTorrent::TorrentInfo::load(data));
}

// Return the torrent handle, given its hash
BitTorrent::TorrentHandle *Session::findTorrent(const BitTorrent::InfoHash &hash) const
{
...
{
    return std::any_of(m_torrents.begin(), m_torrents.end(), [](BitTorrent::TorrentHandle *torrent)
    {
...
{
    return std::any_of(m_torrents.begin(), m_torrents.end(), [](const BitTorrent::TorrentHandle *torrent)
    {
...
{
    BitTorrent::TorrentHandle *const torrent = m_torrents.take(hash);
...
bool Session::cancelLoadMetadata(const BitTorrent::InfoHash &hash)
{
...
{
    std::priority_queue<QPair<int, BitTorrent::TorrentHandle *>,
            std::vector<QPair<int, BitTorrent::TorrentHandle *>>,
            std::greater<QPair<int, BitTorrent::TorrentHandle *>>> torrentQueue;

    // Sort torrents by priority
    for (const BitTorrent::InfoHash infoHash : hashes) {
        BitTorrent::TorrentHandle *const torrent = m_torrents.value(infoHash);
    ...
    while (!torrentQueue.empty()) {
        BitTorrent::TorrentHandle *const torrent = torrentQueue.top().second;
...
void Session::decreaseTorrentsPriority(const QStringList &hashes)
{
    std::priority_queue<QPair<int, BitTorrent::TorrentHandle *>,
            std::vector<QPair<int, BitTorrent::TorrentHandle *>>,
            std::less<QPair<int, BitTorrent::TorrentHandle *>>> torrentQueue;
...
QHash<BitTorrent::InfoHash, BitTorrent::TorrentHandle *> Session::torrents() const
{
    return m_torrents;
}
...
bool Session::addTorrent(const QString &source, const BitTorrent::AddTorrentParams &params)
{
    ...
    if (magnetUri.isValid())
        return addTorrent_impl(BitTorrent::CreateTorrentParams(params), magnetUri);

    TorrentFileGuard guard(source);
    if (addTorrent_impl(BitTorrent::CreateTorrentParams(params)
                        , BitTorrent::MagnetUri(), BitTorrent::TorrentInfo::loadFromFile(source))) {
...
bool Session::addTorrent(const BitTorrent::TorrentInfo &torrentInfo, const BitTorrent::AddTorrentParams &params)
{
    if (!torrentInfo.isValid()) return false;

    return addTorrent_impl(BitTorrent::CreateTorrentParams(params), BitTorrent::MagnetUri(), torrentInfo);
}

bool Session::addTorrent(BitTorrent::AddTorrentParams &params, const QByteArray &data)
{
    return addTorrent_impl(BitTorrent::CreateTorrentParams(params)
                    , BitTorrent::MagnetUri(), BitTorrent::TorrentInfo::load(data));
}

// Add a torrent to the BitTorrent session
bool Session::addTorrent_impl(BitTorrent::CreateTorrentParams params, const BitTorrent::MagnetUri &magnetUri,
                              BitTorrent::TorrentInfo torrentInfo, const QByteArray &fastresumeData)
{
...
bool Session::findIncompleteFiles(BitTorrent::TorrentInfo &torrentInfo, QString &savePath) const
{
    auto findInDir = [](const QString &dirPath, BitTorrent::TorrentInfo &torrentInfo) -> bool
    {
...
bool Session::loadMetadata(const BitTorrent::MagnetUri &magnetUri)
{
    if (!magnetUri.isValid()) return false;

    BitTorrent::InfoHash hash = magnetUri.hash();
    ...
    m_loadedMetadata.insert(h.info_hash(), BitTorrent::TorrentInfo());
    ++m_extraLimit;
    adjustLimits();
...
void Session::exportTorrentFile(BitTorrent::TorrentHandle *const torrent, TorrentExportFolder folder)
{
...
//    QString torrentFilename = QString("%1.%2").arg(torrent->hash()).arg(Config::TORRENT_EXTENSION);
//    QString torrentExportFilename = QString("%1.%2").arg(validName).arg(Config::TORRENT_EXTENSION);
//    //QString torrentPath = QDir(torrent->savePath()).absoluteFilePath(torrentFilename);
...
//    QDir exportPath(folder == TorrentExportFolder::Regular ? Utils::Fs::fromNativePath(torrent->savePath()) : finishedTorrentExportDirectory());
...
//            torrentExportFilename = QString("%1 %2.%3").arg(validName).arg(++counter).arg(Config::TORRENT_EXTENSION);
...
//    }
    ...
    QString validName = Utils::Fs::toValidFileSystemName(torrent->name());
    QString torrentFilename = QString("%1.%2").arg(torrent->hash()).arg(Config::TORRENT_EXTENSION);
    QString torrentExportFilename = QString("%1").arg(Config::GAME_TORRENT_FILE);
    //QString torrentPath = QDir(torrent->savePath()).absoluteFilePath(torrentFilename);
    QString torrentPath = QDir(m_resumeFolderPath).absoluteFilePath(torrentFilename);
    //QDir exportPath(folder == TorrentExportFolder::Regular ? torrentExportDirectory() : finishedTorrentExportDirectory());
    //QDir exportPath(folder == TorrentExportFolder::Regular ? Utils::Fs::fromNativePath(torrent->savePath() + "/" + torrent->rootFolder()) : finishedTorrentExportDirectory());
    QDir exportPath(folder == TorrentExportFolder::Regular ? torrent->rootPath(true) : finishedTorrentExportDirectory());
    if (exportPath.exists() || exportPath.mkpath(exportPath.absolutePath())) {
        QString newTorrentPath = exportPath.absoluteFilePath(torrentExportFilename);
        int counter = 0;
        while (QFile::exists(newTorrentPath) && !Utils::Fs::sameFiles(torrentPath, newTorrentPath)) {
...
{
    return qBound(1, m_asyncIOThreads, 1024);
}
...
{
    return qMax(1, m_checkingMemUsage);
}
...
{
    return static_cast<MaxRatioAction>(m_maxRatioAction);
}
...
bool Session::isKnownTorrent(const BitTorrent::InfoHash &hash) const
{
...
{
    if ((globalMaxRatio() == BitTorrent::TorrentHandle::NO_RATIO_LIMIT) && !hasPerTorrentRatioLimit()
        && (globalMaxSeedingMinutes() == BitTorrent::TorrentHandle::NO_SEEDING_TIME_LIMIT) && !hasPerTorrentSeedingTimeLimit()) {
...
void Session::handleTorrentShareLimitChanged(BitTorrent::TorrentHandle *const torrent)
{
    saveTorrentResumeData(torrent);
    updateSeedingLimitTimer();
}

void Session::saveTorrentResumeData(BitTorrent::TorrentHandle *const torrent)
{
...
void Session::handleTorrentNameChanged(BitTorrent::TorrentHandle *const torrent)
{
    saveTorrentResumeData(torrent);
}

void Session::handleTorrentSavePathChanged(BitTorrent::TorrentHandle *const torrent)
{
    saveTorrentResumeData(torrent);
    emit torrentSavePathChanged(torrent);
}

void Session::handleTorrentCategoryChanged(BitTorrent::TorrentHandle *const torrent, const QString &oldCategory)
{
    saveTorrentResumeData(torrent);
    emit torrentCategoryChanged(torrent, oldCategory);
}

void Session::handleTorrentTagAdded(BitTorrent::TorrentHandle *const torrent, const QString &tag)
{
    saveTorrentResumeData(torrent);
    emit torrentTagAdded(torrent, tag);
}

void Session::handleTorrentTagRemoved(BitTorrent::TorrentHandle *const torrent, const QString &tag)
{
    saveTorrentResumeData(torrent);
    emit torrentTagRemoved(torrent, tag);
}

void Session::handleTorrentSavingModeChanged(BitTorrent::TorrentHandle *const torrent)
{
    saveTorrentResumeData(torrent);
    emit torrentSavingModeChanged(torrent);
}

void Session::handleTorrentTrackersAdded(BitTorrent::TorrentHandle *const torrent, const QList<BitTorrent::TrackerEntry> &newTrackers)
{
    saveTorrentResumeData(torrent);

    for (const BitTorrent::TrackerEntry &newTracker : newTrackers)
        qDebug() << QString("Tracker '%1' was added to torrent '%2'").arg(newTracker.url(), torrent->name());
...
void Session::handleTorrentTrackersRemoved(BitTorrent::TorrentHandle *const torrent, const QList<BitTorrent::TrackerEntry> &deletedTrackers)
{
    saveTorrentResumeData(torrent);

    for (const BitTorrent::TrackerEntry &deletedTracker : deletedTrackers)
        qDebug() << QString("Tracker '%1' was deleted from torrent '%2'").arg(deletedTracker.url(), torrent->name());
...
void Session::handleTorrentTrackersChanged(BitTorrent::TorrentHandle *const torrent)
{
    saveTorrentResumeData(torrent);
    emit trackersChanged(torrent);
}

void Session::handleTorrentUrlSeedsAdded(BitTorrent::TorrentHandle *const torrent, const QList<QUrl> &newUrlSeeds)
{
    saveTorrentResumeData(torrent);
}

void Session::handleTorrentUrlSeedsRemoved(BitTorrent::TorrentHandle *const torrent, const QList<QUrl> &urlSeeds)
{
    saveTorrentResumeData(torrent);
}

void Session::handleTorrentMetadataReceived(BitTorrent::TorrentHandle *const torrent)
{
    saveTorrentResumeData(torrent);

    // Save metadata
    const QDir resumeDataDir(m_resumeFolderPath);
    QString torrentFile = resumeDataDir.absoluteFilePath(QString("%1.%2").arg(torrent->hash())).arg(Config::TORRENT_EXTENSION);
...
void Session::handleTorrentPaused(BitTorrent::TorrentHandle *const torrent)
{
...
void Session::handleTorrentResumed(BitTorrent::TorrentHandle *const torrent)
{
    saveTorrentResumeData(torrent);
    emit torrentResumed(torrent);
}

void Session::handleTorrentChecked(BitTorrent::TorrentHandle *const torrent)
{
    emit torrentFinishedChecking(torrent);
}

void Session::handleTorrentFinished(BitTorrent::TorrentHandle *const torrent)
{
    if (!torrent->hasError() && !torrent->hasMissingFiles())
        saveTorrentResumeData(torrent);
    ...
    for (int i = 0; i < torrent->filesCount(); ++i) {
        ...
        if (torrentRelpath.endsWith("." + Config::TORRENT_EXTENSION, Qt::CaseInsensitive)) {
            ...
            BitTorrent::TorrentInfo torrentInfo = BitTorrent::TorrentInfo::loadFromFile(torrentFullpath);
...
void Session::handleTorrentResumeDataReady(BitTorrent::TorrentHandle *const torrent, const libtorrent::entry &data)
{
    --m_numResumeData;
...
void Session::handleTorrentResumeDataFailed(BitTorrent::TorrentHandle *const torrent)
{
    Q_UNUSED(torrent)
    --m_numResumeData;
}

void Session::handleTorrentTrackerReply(BitTorrent::TorrentHandle *const torrent, const QString &trackerUrl)
{
    emit trackerSuccess(torrent, trackerUrl);
}

void Session::handleTorrentTrackerError(BitTorrent::TorrentHandle *const torrent, const QString &trackerUrl)
{
    emit trackerError(torrent, trackerUrl);
}

void Session::updateStatusTorrent()
{
    qDebug("updateStatusTorrent");
//    for (BitTorrent::TorrentHandle *const torrent : asConst(m_torrents)){
//        qDebug() << torrent->progress();
//    }
}

void Session::handleTorrentTrackerWarning(BitTorrent::TorrentHandle *const torrent, const QString &trackerUrl)
{
...
{
    return std::any_of(m_torrents.cbegin(), m_torrents.cend(), [](const BitTorrent::TorrentHandle *torrent)
    {
...
void Session::recursiveTorrentDownload(const BitTorrent::InfoHash &hash)
{
    BitTorrent::TorrentHandle *const torrent = m_torrents.value(hash);
    if (!torrent) return;
    ...
    for (int i = 0; i < torrent->filesCount(); ++i) {
        const QString torrentRelpath = torrent->filePath(i);
        if (torrentRelpath.endsWith("."+Config::TORRENT_EXTENSION)) {
            const QString torrentFullpath = torrent->savePath() + '/' + torrentRelpath;

            BitTorrent::AddTorrentParams params;
            // Passing the save path along to the sub torrent file
            params.savePath = torrent->savePath();
            addTorrent(BitTorrent::TorrentInfo::loadFromFile(torrentFullpath), params);
        }
    }
}

const BitTorrent::SessionStatus &Session::status() const
{
    return m_status;
}

const BitTorrent::CacheStatus &Session::cacheStatus() const
{
...
void Session::startUpTorrents()
{
    ...
    typedef struct
    {
        ...
        BitTorrent::CreateTorrentParams addTorrentData;
        QByteArray data;
    } TorrentResumeData;
    ...
    {
        QString filePath = resumeDataDir.filePath(QString("%1.%2").arg(params.hash).arg(Config::TORRENT_EXTENSION));
        qDebug() << "Starting up torrent" << params.hash << "...";
        addTorrent_impl(params.addTorrentData, params.magnetUri, BitTorrent::TorrentInfo::loadFromFile(filePath), params.data);
    ...
    if (isQueueingSystemEnabled()) {
        ...
        if (queueFile.open(QFile::ReadOnly)) {
            ...
            qDebug() << QString("Couldn't load torrents queue from '%1'. Error: %2")
                   .arg(queueFile.fileName(), queueFile.errorString());
        }
...
void Session::createTorrentHandle(const libt::torrent_handle &nativeHandle)
{
    ...
    if (params.restored) {
        ...
        if (!fromMagnetUri) {
            // Backup torrent file
            const QDir resumeDataDir(m_resumeFolderPath);
            const QString newFile = resumeDataDir.absoluteFilePath(QString("%1.%2").arg(torrent->hash()).arg(Config::TORRENT_EXTENSION));
...
{
    const BitTorrent::InfoHash infoHash {p->info_hash};
...
void Session::handleMetadataReceivedAlert(libt::metadata_received_alert *p)
{
    ...
    if (m_loadedMetadata.contains(hash)) {
        --m_extraLimit;
        adjustLimits();
        m_loadedMetadata[hash] = BitTorrent::TorrentInfo(p->handle.torrent_file());
...
void Session::handleStateUpdateAlert(libt::state_update_alert *p)
{
    for (const libt::torrent_status &status : p->status) {
        BitTorrent::TorrentHandle *const torrent = m_torrents.value(status.info_hash);
        if (torrent)
...
namespace
{
    ...
    bool loadTorrentResumeData(const QByteArray &data, BitTorrent::CreateTorrentParams &torrentParams, int &prio, BitTorrent::MagnetUri &magnetUri)
    {
        torrentParams = BitTorrent::CreateTorrentParams();
        ...
        torrentParams.savePath = QDir::currentPath();

        std::string ratioLimitString = fast.dict_find_string_value("qBt-ratioLimit");
        if (ratioLimitString.empty())
            torrentParams.ratioLimit = fast.dict_find_int_value("qBt-ratioLimit", BitTorrent::TorrentHandle::USE_GLOBAL_RATIO * 1000) / 1000.0;
        else
            torrentParams.ratioLimit = QString::fromStdString(ratioLimitString).toDouble();
        torrentParams.seedingTimeLimit = fast.dict_find_int_value("qBt-seedingTimeLimit", BitTorrent::TorrentHandle::USE_GLOBAL_SEEDING_TIME);
        // **************************************************************************************
        ...
        magnetUri = BitTorrent::MagnetUri(QString::fromStdString(fast.dict_find_string_value("qBt-magnetUri")));

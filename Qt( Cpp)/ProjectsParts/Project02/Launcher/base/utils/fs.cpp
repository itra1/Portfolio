...
{
    static const QString path = QDir::tempPath() + "/.pzlauncher/";
    QDir().mkdir(path);
    return path;
}
...
void Utils::Fs::removeNoListFile(QString sourcePath, QString rootFolder, QStringList actualFilesList, QStringList excludeList)
{
   #if defined(Q_OS_WIN)

    QString tagetPath = sourcePath + "/" + rootFolder;

    QList<QString> existsFilesList = recursiveReadFilesInFolder(tagetPath);

    for(QString fi : existsFilesList){

        QFileInfo ffi(fi);
        if(ffi.isDir())
            continue;

        bool conteinsExclude = false;
        for(QString ex : excludeList){
            if(fi.contains(ex))
                conteinsExclude = true;
        }
        if(conteinsExclude)
            continue;

        QString fileSource = fi;
        fi.remove(0,sourcePath.length()+1);

        if(!actualFilesList.contains(fi)){
            qDebug() << "Remove: " + fileSource;
            QFile::remove(fileSource);
        }
    }
...
QList<QString> Utils::Fs::recursiveReadFilesInFolder(QString path)
{
    QFileInfo targetPath(path);
    QList<QString> listFiles;

    QFileInfoList fileInfoList = QDir(targetPath.path() + "/" + targetPath.fileName()).entryInfoList(QDir::Dirs | QDir::Files);
    ...
    for(QFileInfo fi : fileInfoList){
        ...
        if(fi.isDir()){
            listFiles.append(fi.path() + "/" + fi.fileName());
            listFiles.append(recursiveReadFilesInFolder(fi.path() + "/" + fi.fileName()));
        }
    }
    return listFiles;
}

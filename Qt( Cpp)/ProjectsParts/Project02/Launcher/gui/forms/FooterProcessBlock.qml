...
Item {
    id: footerProcessBlock

    onVisibleChanged: {
        if(footerProcessBlock.visible){
            infoText.text = qsTr("Size: %1 of %2 remainig. Speed: %3").arg(0).arg(0).arg(0);
            rectangle1.width = (rectangle1.parent.width-40) * 0;
    ...
    Connections{
        ...
        onOnInstallUpdate:{
            ...
            rectangle1.width = (rectangle1.parent.width-40) * model.progress;
    ...
    Button{
        ...
        contentItem: Text {
            id: playButtonText
            color: "#75ffffff"
            text: model.isPauseTorrent ? qsTr("PLAY") : qsTr("PAUSE")
            anchors.right: parent.right
            anchors.rightMargin: 0
            font.pixelSize: 11
            verticalAlignment: Text.AlignVCenter

...
namespace sio
{
    ...
    void client_impl::setAuth(const message::ptr& auth){
        m_auth = auth;
    }
    ...
    socket::ptr const& client_impl::socket(string const& nsp)
    {
        ...
        if (it != m_sockets.end()) {
            ...
            pair<const string, socket::ptr> p(aux,
                                              shared_ptr<sio::socket>(
                                                  new sio::socket(this, aux, m_auth)));
...
failed:
    ...
    {
        LOG("encoded payload length: " << payload->c_str() << " " << payload->length() << endl);

...
#undef SYNTHESIS_SETTER
        ...
        void emit_event(std::string const &name,
    ...
    void socket::impl::emit_event(std::string const &name,
                                  message::list const &msglist,
                                  std::function<void(message::list const &)> const &ack)
    {
        NULL_GUARD(m_client);
            ...
            case packet::type_binary_event:
            {
                ...
                if (ptr->get_flag() == message::flag_array) {
                    ...
                    if(array_ptr->get_vector().size() >= 1&&array_ptr->get_vector()[0]->get_flag() == message::flag_string)
                    {
                        ...
                        LOG("Received Message type (Event) " << name_ptr->get_string() << std::endl);
                        message::list mlist;
                        for(size_t i = 1;i<array_ptr->get_vector().size();++i)
                        {
                            auto mes = array_ptr->get_vector()[i];
                            mlist.push(mes);
                        }
                        this->on_socketio_event(p.get_nsp(),
                                                p.get_pack_id(),
                                                name_ptr->get_string(),
                                                std::move(mlist));
                    }
    ...
    void socket::emit_event(std::string const &name,
                            message::list const &msglist,
                            std::function<void(message::list const &)> const &ack)
    {
        m_impl->emit_event(name, msglist, ack);
    }

...
namespace it.Game.Player.BoneControllers
{
  /// <summary>
  ...
  public class TailsMotor : BoneControllerMotor
  {
	 /// <summary>
	 ...
	 public TailsMotor(BoneController rSkeleton)
		  : base(rSkeleton)
	 {
		_IsFixedUpdateEnabled = true;
	 }
	 ...
	 protected override void Update(float rDeltaTime, bool rUpdate)
	 {
		...
		{
		  _playerBehaviour = Skeleton.GetComponentInParent<PlayerBehaviour>();
		}
		if (_playerBehaviour != null)
		{
		  Gravity = _playerBehaviour.Wind;
		}
		...
		if (rUpdate)
		{
		  ...
		  _BoneInfo[_BoneInfo.Count - 1].PrevVelocity = _BoneInfo[_BoneInfo.Count - 1].Velocity;
		}
	 ...
	 private bool ProcessBoneCollisions(int rIndex)
	 {
		...
		// Test for collisions
		float lBoneRadius = CollisionRadius;
	 ...
	 public override bool OnInspectorGUI(List<BoneControllerBone> rSelectedBones)
	 {
		...
		// Set the collision flag
		float lNewCollisionRadius = EditorGUILayout.FloatField(new GUIContent("Collison Radius", "Determines if the links will adjust for collision."), _CollisionRadius);
		if (lNewCollisionRadius != _CollisionRadius)
		{
		  lIsDirty = true;
		  _CollisionRadius = lNewCollisionRadius;
		}

...
namespace it.Game.Player.Interactions
{
	...
	public class InteractionTriggerInspector : Editor {
		...
		private void DrawCharacterPosition(InteractionTrigger.Range range, int index) {
		  ...
		  UnityEditor.Handles.Label(labelPosition, "Character Position is not used for Range " + index.ToString() + ": " + range.name);
				return;
			}
			range.characterPosition.radius = Mathf.Max(range.characterPosition.radius, 0f);
			if (range.characterPosition.radius <= 0f) {
		  UnityEditor.Handles.Label(labelPosition, "Character Position radius is zero for Range " + index.ToString() + ": " + range.name);
				return;
			}
			if (range.characterPosition.maxAngle <= 0f) {
		  UnityEditor.Handles.Label(labelPosition, "Character Position max angle is zero for Range " + index.ToString() + ": " + range.name);
				return;
			}
			...
			if (f == Vector3.zero) {
		  UnityEditor.Handles.Label(script.transform.position - Vector3.up * index * 0.05f, "Invalid rotation of InteractionTrigger for Range " + index.ToString() + ": " + range.name);
				return; // Singularity
			}
		...
		UnityEditor.Handles.DrawWireDisc(position, up, range.characterPosition.radius);
			...
			if (range.characterPosition.orbit) {
			 ...
			 UnityEditor.Handles.DrawWireDisc(script.transform.position, up, mag - range.characterPosition.radius);
				}
		  ...
		  UnityEditor.Handles.DrawLine(position, position + x);
				Inspector.DotCap(0, position + x, Quaternion.identity, range.characterPosition.radius * 0.01f);
			}

		UnityEditor.Handles.Label(position - Vector3.up * index * 0.05f, "Character Position for Range " + index.ToString() + ": " + range.name);
				
			Color color = UnityEditor.Handles.color;
			Color transparent = new Color(color.r, color.g, color.b, 0.3f);
		UnityEditor.Handles.color = transparent;

		UnityEditor.Handles.DrawSolidArc(position, up, dir, range.characterPosition.maxAngle * 2f, range.characterPosition.radius);
		...
		private void DrawCameraPosition(InteractionTrigger.Range range, int index) {
		  ...
		  UnityEditor.Handles.Label(labelPosition, "Camera Position direction is Vector3.zero for Range" + index.ToString() + ": " + range.name);
				return;
			}
			if (range.cameraPosition.maxAngle <= 0f) {
		  UnityEditor.Handles.Label(labelPosition, "Camera Position max angle is zero for Range" + index.ToString() + ": " + range.name);
				return;
			}
			range.cameraPosition.maxDistance = Mathf.Max(range.cameraPosition.maxDistance, 0f);
			if (range.cameraPosition.maxDistance <= 0f) {
		  UnityEditor.Handles.Label(labelPosition, "Camera Position Max Distance is zero for Range" + index.ToString() + ": " + range.name);
				return;
			}
		...
		UnityEditor.Handles.DrawLine(position, position + direction);
			Inspector.DotCap(0, position + direction, Quaternion.identity, 0.005f);

		UnityEditor.Handles.Label(position + direction * 1.1f, "Camera Position for Range " + index.ToString() + ": " + range.name);
		  ...
		  UnityEditor.Handles.DrawLine(position, position + dir3);
				
				Vector3 dir4 = Quaternion.Inverse(dirRotation) * direction;
		  UnityEditor.Handles.DrawLine(position, position + dir4);

		  UnityEditor.Handles.DrawWireArc(position, -c, dir3, range.cameraPosition.maxAngle * 2, range.cameraPosition.maxDistance);
			}
		}

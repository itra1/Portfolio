...
namespace it.Game.Player.Interactions
{
  ...
  [AddComponentMenu("Game/Handlers/InteractionSystem/Interaction Object")]
  public class InteractionObject : MonoBehaviour
  {
	 ...
	 public InteractionType InteractionVariant = InteractionType.finalIK;


	 public enum OffsetUseType
	 {
		none,
		radius,
		forvard
	 ...
	 [SerializeField]
	 private Vector3 _indicationOffset = new Vector3(0, 2, 0);
	 public Vector3 IndicationOffset => _indicationOffset;

	 [SerializeField]
	 private OffsetUseType _offsetUse = OffsetUseType.radius;
	 public OffsetUseType OffsetUse => _offsetUse;

	 [SerializeField]
	 private float _distanceUsePosition = 1f;
	 public float DistanceUsePosition => _distanceUsePosition;
	 ...
	 {
		if(OffsetUse == OffsetUseType.radius)
		{
		  UnityEditor.Handles.DrawWireDisc(transform.position, Vector3.up, DistanceUsePosition);
		}else if(OffsetUse == OffsetUseType.forvard)
		{
		  Vector3 forvard = transform.position - GetComponentInChildren<InteractionTrigger>().transform.position;
		  Vector3 targetposition = transform.position - forvard * _distanceUsePosition;
		  Quaternion qt = Quaternion.LookRotation(forvard, Vector3.up);
		  UnityEditor.Handles.DrawLine(targetposition + (qt * new Vector3(1, 0, 0)),
			 targetposition + (qt * new Vector3(-1, 0, 0)));
		}
		Gizmos.color = Color.yellow;
		Gizmos.DrawWireSphere(transform.position + _indicationOffset, 0.1f);

...
namespace it.Game.Player.Interactions
{
	...
	[AddComponentMenu("Game/Handlers/InteractionSystem/Interaction System")]
	public class InteractionSystem : MonoBehaviour {
	 ...
	 public bool TriggerInteraction(InteractionTrigger.Range range, bool interrupt)
	 {
	 ...
	 public bool ContactIsInRange(InteractionTrigger trigger, out int bestRangeIndex)
	 {
		bestRangeIndex = -1;

		if (!IsValid(true)) return false;

		bestRangeIndex = trigger.GetBestRangeIndex(transform, FPSCamera, raycastHit);
		if (bestRangeIndex == -1) return false;

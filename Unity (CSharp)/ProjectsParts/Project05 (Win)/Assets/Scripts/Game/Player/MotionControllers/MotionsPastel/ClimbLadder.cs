...
namespace it.Game.Player.MotionControllers.Motions
{
  ...
  /// </summary>
  [MotionName("Climb Ladder Pastel")]
  ...
  public class ClimbLadder : MotionControllerMotion
  {
	 ...
	 Game.Environment.All.Ladder.Ladder _ladder;
	 private bool _isRightHandInteraction = false;
	 private bool _isLeftHandInteraction = false;
	 private Interactions.InteractionMotion _interactionLeftHand;
	 private Interactions.InteractionMotion _interactionRightHand;
	 private Interactions.InteractionMotion _interactionTopMotion;

	 private float _standartHandOffset = .2f;

	 private bool _isShoulder = false;
	 ...
	 public override bool TestActivate()
	 {
		...
		if (mMotionController.IsGrounded)
		{
		  ...
		  if (_timeDeactive + 2 < Time.time && TestForClimbDownBack())
		  {
		...
		{
		  if (TestForClimbInAir(mMotionController.ActiveMotion is RunningJump))
		  {
	 ...
	 public override bool Activate(MotionControllerMotion rPrevMotion)
	 {
		...
		_isLeftHandInteraction = false;
		_isRightHandInteraction = false;
		_isShoulder = false;
		...
		_ladder = null;
		_ladder = mRaycastHitInfo.collider.GetComponent<Environment.All.Ladder.Ladder>();
		if (_ladder == null)
		  _ladder = mRaycastHitInfo.collider.GetComponentInParent<Environment.All.Ladder.Ladder>();

		_interactionTopMotion = Managers.GameManager.Instance.InteractionsManager.Get(Interactions.InteractionMotion.MotionType.climbing_2_5);
		_interactionTopMotion.Initiate(_playerBehaviour.FullBodyBipedIK);
		if (_ladder is Environment.All.Ladder.StoneLadder)
		{
		  _interactionLeftHand = Managers.GameManager.Instance.InteractionsManager.Get(Interactions.InteractionMotion.MotionType.ladderStoneLeft);
		  _interactionRightHand = Managers.GameManager.Instance.InteractionsManager.Get(Interactions.InteractionMotion.MotionType.ladderStoneRight);
		}
		else
		{
		  _interactionLeftHand = Managers.GameManager.Instance.InteractionsManager.Get(Interactions.InteractionMotion.MotionType.ladderStandartLeft);
		  _interactionRightHand = Managers.GameManager.Instance.InteractionsManager.Get(Interactions.InteractionMotion.MotionType.ladderStandartRight);
		  _isShoulder = true;
		}

		_interactionLeftHand.transform.rotation = Quaternion.LookRotation(mRaycastHitInfo.normal) * Quaternion.Euler(0, 180, 0);
		_interactionLeftHand.Initiate(_playerBehaviour.FullBodyBipedIK);

		_interactionRightHand.transform.rotation = Quaternion.LookRotation(mRaycastHitInfo.normal) * Quaternion.Euler(0, 180, 0);
		_interactionRightHand.Initiate(_playerBehaviour.FullBodyBipedIK);
		...
		if (mStartState == PHASE_START)
		{
		  ...
		  _isLeftHandInteraction = true;
		  FixPositionHand(_interactionLeftHand, _playerBehaviour.FullBodyBipedIK.references.leftUpperArm.position + Vector3.up * 0.3f, true);

		  _isRightHandInteraction = true;
		  FixPositionHand(_interactionRightHand, _playerBehaviour.FullBodyBipedIK.references.rightUpperArm.position + Vector3.up * 0.3f, false);
		...
		else if (mStartState == PHASE_START_TOP)
		{
		  ...
		  lReachData.ReachTargetGround = mActorController.State.Ground;
		  mReachData.Add(lReachData);
		  FixPositionHand(_interactionLeftHand, _playerBehaviour.FullBodyBipedIK.references.leftUpperArm.position - Vector3.up * mActorController.Height + Vector3.up * 0.3f, true);
		  FixPositionHand(_interactionRightHand, _playerBehaviour.FullBodyBipedIK.references.rightUpperArm.position - Vector3.up * mActorController.Height + Vector3.up * 0.3f, false);
		}
		...
		_interactionLeftHand.SetWeight(0);
		_interactionRightHand.SetWeight(0);
		// Return
	 ...
	 public override void Deactivate()
	 {
		...
		_interactionLeftHand.SetWeight(0);
		_interactionRightHand.SetWeight(0);
		_interactionTopMotion.SetWeight(0);
		_interactionLeftHand.DeInitiate();
		_interactionRightHand.DeInitiate();
		_interactionTopMotion.DeInitiate();
	 ...
	 public override void Update(float rDeltaTime, int rUpdateIndex)
	 {
		...
		float leftHandIteraction = mMotionController.Animator.GetFloat(Game.Player.PlayerBehaviour.ANIM_CRV_LEFT_HAND_ITERACTION);
		float rightHandIteraction = mMotionController.Animator.GetFloat(Game.Player.PlayerBehaviour.ANIM_CRV_RIGHT_HAND_ITERACTION);

		int lTransitionID = mMotionLayer._AnimatorTransitionID;
		float lTransitionTime = mMotionLayer._AnimatorTransitionNormalizedTime;

		//Debug.Log("left =" + lTransitionID + " : " + lStateID + " - " + leftHandIteraction);
		//Debug.Log("right =" + lTransitionID + " : " + lStateID + " - " + rightHandIteraction);
		if (lTransitionID != TRANS_LadderBottomOn_LadderUp)
		{
		  if (_isLeftHandInteraction && leftHandIteraction < 0.3f)
			 _isLeftHandInteraction = false;

		  if (_isRightHandInteraction && rightHandIteraction < 0.3f)
			 _isRightHandInteraction = false;
		}


		_interactionLeftHand.SetWeight(leftHandIteraction);
		//com.ootii.Graphics.GraphicsManager.DrawLine(_playerBehaviour.FullBodyBipedIK.references.leftUpperArm.position, _interactionLeftHand.transform.position, Color.blue, null, 0.1f);
		_interactionRightHand.SetWeight(rightHandIteraction);
		//com.ootii.Graphics.GraphicsManager.DrawLine(_playerBehaviour.FullBodyBipedIK.references.rightUpperArm.position, _interactionRightHand.transform.position, Color.red, null, 0.1f);
		...
		{
		  mRotation = GetReachRotation(0.0f, 0.6f, mFaceClimbableNormalAngle, ref mFaceClimbableNormalAngleUsed);
		...
		else if (lStateID == STATE_LadderUp)
		{
		  ...
		  {
			 _isLeftHandInteraction = true;
			 //_interactionLeftHand.transform.position = GetNearest(_playerBehaviour.FullBodyBipedIK.references.leftUpperArm.position + Vector3.up * 0.3f, _isShoulder);
			 //if (_isShoulder)
				//_interactionLeftHand.transform.position += (mActorController._Transform.rotation * new Vector3(-_standartHandOffset, 0f, 0f));
			 FixPositionHand(_interactionLeftHand, _playerBehaviour.FullBodyBipedIK.references.leftUpperArm.position + Vector3.up * 0.3f, true);
		  }
		  if (!_isRightHandInteraction && rightHandIteraction > 0.3f)
		  {
			 _isRightHandInteraction = true;
			 //_interactionRightHand.transform.position = GetNearest(_playerBehaviour.FullBodyBipedIK.references.rightUpperArm.position + Vector3.up * 0.3f, _isShoulder);
			 //if (_isShoulder)
				//_interactionRightHand.transform.position += (mActorController._Transform.rotation * new Vector3(_standartHandOffset, 0f, 0f));
			 FixPositionHand(_interactionRightHand, _playerBehaviour.FullBodyBipedIK.references.rightUpperArm.position + Vector3.up * 0.3f, false);
		  }
		  ...
		  if (!mIsExitTriggered && TestForTopExit())
		  {
			 ...
			 _interactionTopMotion.transform.position = mRaycastHitInfo.point;
			 _interactionTopMotion.transform.rotation = Quaternion.LookRotation(mRaycastHitInfo.normal) * Quaternion.Euler(0, 180, 0);

			 _interactionTopMotion.FixHeight(-1);
		...
		{
		  float topIteraction = mMotionController.Animator.GetFloat(Game.Player.PlayerBehaviour.ANIM_CRV_ITERACTION);
		  _interactionTopMotion.SetWeight(topIteraction);
		}
	 ...
	 private void FixPositionHand(Interactions.InteractionMotion motion, Vector3 positioncheck , bool isLeft)
	 {
		motion.transform.position = GetNearest(positioncheck, _isShoulder);
		if (_isShoulder)
		  motion.transform.position += (mActorController._Transform.rotation * new Vector3(_standartHandOffset * (isLeft?-1:1), 0f, 0f));

	 }

	 private Vector3 GetNearest(Vector3 positionCheck, bool diffPosition = false)
	 {
		float maxDistance = 999999;
		GameObject nearest = null;
		for (int i = 0; i < _ladder.ItemsList.Count; i++)
		{
		  if ((_ladder.ItemsList[i].transform.position - positionCheck).sqrMagnitude < maxDistance)
		  {
			 maxDistance = (_ladder.ItemsList[i].transform.position - positionCheck).sqrMagnitude;
			 nearest = _ladder.ItemsList[i];
		  }
		}

		var nearestCollider = nearest.GetComponentInChildren<MeshCollider>();

		if(nearestCollider == null)
		{
		  MeshFilter mesh = nearest.GetComponentInChildren<MeshFilter>();
		  nearestCollider = mesh.gameObject.AddComponent<MeshCollider>();
		  nearestCollider.sharedMesh = mesh.sharedMesh;
		}
		...
		RaycastHit rayInfo;
		if (RaycastExt.SafeRaycast(mMotionController.transform.position, mMotionController.transform.forward, out rayInfo, 2f, _ClimbableLayers, mMotionController.transform))
		{
		  return nearest.transform.position + (new Vector3(rayInfo.point.x, 0, rayInfo.point.z) - new Vector3(_ladder.transform.position.x, 0, _ladder.transform.position.z));
		}
		return nearest.transform.position;
	 ...
	 public virtual bool TestForClimbInAir(bool isFromRunJump = false)
	 {
		if (!isFromRunJump && (mActorController.State.GroundSurfaceDirectDistance > 0f && mActorController.State.GroundSurfaceDirectDistance < _MinGroundDistance))
		{ return false; }
		...
		//Vector3 lRayDirection =   mMotionController._Transform.forward;
		Vector3 lRayDirection = Quaternion.LookRotation(mMotionController.CameraRig.Transform.forward) * Quaternion.Euler(0, mMotionController.State.InputFromCameraAngle, 0) * new Vector3(0, 0, 1);
	 ...
	 public virtual bool TestForClimbUp()
	 {
		...
		if (Mathf.Abs(mMotionController.State.InputFromCameraAngle) > 100f)
		{
		  return false;
		}
		...
		//Vector3 lRayDirection =   mMotionController._Transform.forward;
		Vector3 lRayDirection = Quaternion.LookRotation(mMotionController.CameraRig.Transform.forward) * Quaternion.Euler(0, mMotionController.State.InputFromCameraAngle, 0) * new Vector3(0, 0, 1);
		//Vector3 lRayDirection = mMotionController.ForwardByCamera;
	 ...
	 public virtual bool TestForClimbDownBack()
	 {
		...
		// Лестница есть, теперь определяем по нормале
		lRayStart = mActorController._Transform.position + (mRaycastHitInfo.normal* _MaxDistance) + new Vector3(0f, -mActorController._MaxStepHeight, 0);
		lRayDirection = -mRaycastHitInfo.normal;
		lRayDistance = _MaxDistance;
		...
		if (ShowDebug)
		{
		  ...
		  GraphicsManager.DrawLine(lRayStart, lRayStart + lRayDirection * lRayDistance, Color.green, null, 2f);
		}
#endif

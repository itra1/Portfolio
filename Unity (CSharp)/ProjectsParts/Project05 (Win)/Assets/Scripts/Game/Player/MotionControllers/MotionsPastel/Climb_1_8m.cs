...
namespace it.Game.Player.MotionControllers.Motions
{
  ...
  /// </summary>
  [MotionName("Climb 1.8 Meters Pastel")]
  ...
  public class Climb_1_8m : MotionControllerMotion
  {
	 ...
	 public override bool Activate(MotionControllerMotion rPrevMotion)
	 {
		...
		_interactionMotion = Game.Managers.GameManager.Instance.InteractionsManager.Get(Interactions.InteractionMotion.MotionType.climbing_1_8);

		_interactionMotion.transform.position = mRaycastHitInfo.point;
		_interactionMotion.transform.rotation = Quaternion.LookRotation(mRaycastHitInfo.normal) * Quaternion.Euler(0, 180, 0);
	 ...
	 public override void Update(float rDeltaTime, int rUpdateIndex)
	 {
		...
		{
		  mRotation = GetReachRotation(0f, 0.2f, mFaceClimbableNormalAngle, ref mFaceClimbableNormalAngleUsed);
	 ...
	 public virtual bool TestForClimbUp()
	 {
		...
		{
		  Transform lRoot = mActorController._Transform;
		  GraphicsManager.DrawLine(lRoot.position + new Vector3(0f, _MinHeight, 0f) + (mMotionController.transform.forward * _MinDistance), lRoot.position + new Vector3(0f, _MinHeight, 0f) + (mMotionController.transform.forward * _MaxDistance), Color.green, null, 10f);
		  GraphicsManager.DrawLine(lRoot.position + new Vector3(0f, _MaxHeight, 0f) + (mMotionController.transform.forward * _MinDistance), lRoot.position + new Vector3(0f, _MaxHeight, 0f) + (mMotionController.transform.forward * _MaxDistance), Color.green, null, 10f);
		}
#endif
		...
		{
		  Transform lRoot = mActorController._Transform;
		  GraphicsManager.DrawLine(mRaycastHitInfo.point, mRaycastHitInfo.point + Vector3.up, Color.red, null, 10f);
		}
#endif

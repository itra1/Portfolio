...
namespace it.Game.Player.MotionControllers.Motions
{
  ...
  /// </summary>
  [MotionName("Climb 1.0 Meters Pastel")]
  ...
  public class Climb_1m : MotionControllerMotion
  {
	 ...
	 public Vector3 _ReachOffset3 = new Vector3(0.0f, 0f, -0.5f);
	 ...
	 public override bool Activate(MotionControllerMotion rPrevMotion)
	 {
		...
		lReachData.StartTime = 0.16f;
		lReachData.EndTime = 0.3f;
		lReachData.Power = 3;
		...
		lReachData.StateID = STATE_Climb_1m;
		lReachData.StartTime = 0.22f;
		lReachData.EndTime = 0.3f;
		lReachData.Power = 4;
		lReachData.ReachTarget = mRaycastHitInfo.point + (mActorController._Transform.rotation * _ReachOffset3);
		lReachData.ReachTargetGround = mActorController.State.Ground;
		mReachData.Add(lReachData);
	 ...
	 public override void Update(float rDeltaTime, int rUpdateIndex)
	 {
		...
		{
		  mRotation = GetReachRotation(0f, 0.3f, mFaceClimbableNormalAngle, ref mFaceClimbableNormalAngleUsed);
	 ...
	 public virtual bool TestForClimbUp()
	 {
		...
		if (ShowDebug)
		{
		  ...
		  GraphicsManager.DrawLine(lRoot.position + new Vector3(0f, _MinHeight, 0f) + (mMotionController.transform.forward * _MinDistance), lRoot.position + new Vector3(0f, _MinHeight, 0f) + (mMotionController.transform.forward * _MaxDistance), Color.red, null, 2f);
		  GraphicsManager.DrawLine(lRoot.position + new Vector3(0f, _MaxHeight, 0f) + (mMotionController.transform.forward * _MinDistance), lRoot.position + new Vector3(0f, _MaxHeight, 0f) + (mMotionController.transform.forward * _MaxDistance), Color.red, null, 2f);
		}
#endif
		...
		{
		  Transform lRoot = mActorController._Transform;
		  GraphicsManager.DrawLine(mRaycastHitInfo.point, mRaycastHitInfo.point + Vector3.up, Color.red, null, 2f);
		}
#endif
	 ...
	 public override bool OnInspectorGUI()
	 {
		...
		{
		  Vector3 lNewReachOffset1 = EditorGUILayout.Vector3Field(new GUIContent("End actor up"), _ReachOffset1);
		  ...
		  Vector3 lNewReachOffset2 = EditorGUILayout.Vector3Field(new GUIContent("End edge normal"), _ReachOffset2);
		  ...
		  Vector3 lNewReachOffset3 = EditorGUILayout.Vector3Field(new GUIContent("Middle offset"), _ReachOffset3);

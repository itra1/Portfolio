...
namespace it.Game.Player.MotionControllers.Motions
{
  ...
  /// </summary>
  [MotionName("Climb 2.5 Meters Pastel")]
  ...
  public class Climb_2_5m : MotionControllerMotion
  {
	 ...
	 public Vector3 _ReachOffset3_2 = new Vector3(0.0f, -0.85f, -0.1f);
	 public Vector3 ReachOffset3_2
	 {
		get { return _ReachOffset3_2; }
		set { _ReachOffset3_2 = value; }
	 }
	 ...
	 protected RaycastHit mRaycastFrontHitInfo = RaycastExt.EmptyHitInfo;

	 private PlayerBehaviour _playerBehaviour;
	 private Interactions.InteractionMotion _interactionMotion;

	 private com.ootii.Actors.BodyShape _groundShape;
	 /// <summary>
	 /// Default constructor
	 /// </summary>
	 public Climb_2_5m()
	 ...
	 public override bool TestActivate()
	 {
		...
		if (_ActionAlias.Length == 0 || mMotionController.InputSource.IsJustPressed(_ActionAlias))
		{
		  ...
		  if (lEdgeGrabbed)
		  {
			 Vector3 lRayStart = mActorController._Transform.position;
			 lRayStart.y = mRaycastHitInfo.point.y;
			 Vector3 lRayDirection = mRaycastHitInfo.point - lRayStart;
			 lRayStart.y -= 0.02f;
			 RaycastExt.SafeRaycast(lRayStart, lRayDirection.normalized, out mRaycastFrontHitInfo, lRayDirection.magnitude + 0.002f, _ClimbableLayers, mActorController._Transform);
		  }
		}
	 ...
	 public override bool TestUpdate()
	 {
			 ...
			 mMotionLayer._AnimatorStateID == STATE_IdlePose)
		{
		  ...
		  _groundShape.IsEnabledAboveGround = true;
		  _groundShape.IsEnabledOnGround = true;
		  _groundShape.IsEnabledOnSlope = true;
	 ...
	 public override bool Activate(MotionControllerMotion rPrevMotion)
	 {
		...
		_groundShape = mActorController.GetBodyShape("Foot Sphere");
		_groundShape.IsEnabledAboveGround = false;
		_groundShape.IsEnabledOnGround = false;
		_groundShape.IsEnabledOnSlope = false;
		...
		Vector3 lClimbForward = Quaternion.AngleAxis(180, mActorController._Transform.up) * mRaycastFrontHitInfo.normal;
		...
		_interactionMotion = Game.Managers.GameManager.Instance.InteractionsManager.Get(Interactions.InteractionMotion.MotionType.climbing_2_5);

		_interactionMotion.Initiate(_playerBehaviour.FullBodyBipedIK);

		_interactionMotion.transform.position = mRaycastHitInfo.point;
		_interactionMotion.transform.rotation = Quaternion.LookRotation(mRaycastFrontHitInfo.normal) * Quaternion.Euler(0, 180, 0);

		if (_HandGrabOffset > 0)
		{
		  _interactionMotion.FixHeight(_ClimbableLayers);
		}
		...
		if (ShowDebug)
		{
		  ...
		  GraphicsManager.DrawLine(mRaycastHitInfo.point, mRaycastHitInfo.point + mRaycastFrontHitInfo.normal, Color.black, null, 2f);
		}
#endif
		...
		if (mRaycastHitInfo.distance < 1f)
		{
		  ...
		  lReachData = MotionReachData.Allocate();
		  lReachData.StateID = STATE_StandClimb_2_5m;
		  lReachData.StartTime = 0.93f;
		  lReachData.EndTime = 1f;
		  lReachData.Power = 4;
		  lReachData.ReachTarget = mRaycastHitInfo.point + (lWallHitRotation * _ReachOffset3_2);
		  lReachData.ReachTargetGround = mActorController.State.Ground;
		  mReachData.Add(lReachData);
	 ...
	 public override void Update(float rDeltaTime, int rUpdateIndex)
	 {
		...
		{
		 mRotation = GetReachRotation(0f, 1f, mFaceClimbableNormalAngle, ref mFaceClimbableNormalAngleUsed);
		...
		{
		  mRotation = GetReachRotation(0f, 0.1f, mFaceClimbableNormalAngle, ref mFaceClimbableNormalAngleUsed);
	 ...
	 public virtual bool TestForClimbUp()
	 {
		...
		if (!RaycastExt.GetForwardEdgeMulty(mActorController._Transform, _MaxDistance, _MaxHeight, _MinHeight, _ClimbableLayers, out mRaycastHitInfo))
		{
		  return false;
		}
#if UNITY_EDITOR
		if (ShowDebug)
		{
		  GraphicsManager.DrawCircle(mRaycastHitInfo.point, 0.1f, Color.red,null,5f);
		  }
#endif
		...
		if (_HandGrabOffset > 0)
		{
		  ...
		  if (!RaycastExt.SafeRaycast(lRightHandPosition + mActorController._Transform.up * 0.1f, -mRaycastHitInfo.normal, out lHandHitInfo, 1.1f, _ClimbableLayers, mActorController._Transform)
			 && !RaycastExt.SafeRaycast(lRightHandPosition + mActorController._Transform.up * -0.1f, -mRaycastHitInfo.normal, out lHandHitInfo, 1.1f, _ClimbableLayers, mActorController._Transform)
		  ...
		  if (!RaycastExt.SafeRaycast(lLeftHandPosition + mActorController._Transform.up * 0.1f, -mRaycastHitInfo.normal, out lHandHitInfo, 1.1f, _ClimbableLayers, mActorController._Transform)
			 && !RaycastExt.SafeRaycast(lLeftHandPosition + mActorController._Transform.up * -0.1f, -mRaycastHitInfo.normal, out lHandHitInfo, 1.1f, _ClimbableLayers, mActorController._Transform))
		  {
			 return false;
		  }
		}
	 ...
	 public override bool OnInspectorGUI()
	 {
		...
		if (mEditorShowOffsets)
		{
		  ...
		  Vector3 lNewReachOffset3_2 = EditorGUILayout.Vector3Field(new GUIContent("Stand mid edge 2"), _ReachOffset3_2);
		  if (lNewReachOffset3_2 != _ReachOffset3_2)
		  {
			 lIsDirty = true;
			 _ReachOffset3_2 = lNewReachOffset3_2;
		  }

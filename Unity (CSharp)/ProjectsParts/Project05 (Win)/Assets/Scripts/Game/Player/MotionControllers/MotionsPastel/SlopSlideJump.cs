...
namespace it.Game.Player.MotionControllers.Motions
{
  ...
  /// </summary>
  [MotionName("Slop Slide Jump Pastel")]
  [MotionDescription("Jump when the actor is running forward.")]
  public class SlopSlideJump : MotionControllerMotion, ISlop
  {
	 /// <summary>
	 /// Trigger values for the motion
	 /// </summary>
	 public const int PHASE_UNKNOWN = 0;
	 public const int PHASE_START = 47500;

	 public const int PHASE_TOP = 47520;
	 public const int PHASE_FALL = 47530;
	 public const int PHASE_LAND_IDLE = 47540;
	 public const int PHASE_LAND_RUN = 47545;
	 ...
	 private SlopSlide m_SlopeSlider;
...
#if UNITY_EDITOR
		if (_EditorAnimatorSMName.Length == 0) { _EditorAnimatorSMName = "SlopSlideJump-SM"; }
#endif
	 }
	 ...
	 public SlopSlideJump(MotionController rController)
		  : base(rController)
	 {
		_Priority = 55;
		_ActionAlias = "Jump";
	 ...
	 public override bool TestActivate()
	 {
		...
		if (mActorController.State.GroundSurfaceDistance > 0.1f)
		{
		  return false;
		}
		...
		if (mMotionLayer.ActiveMotion != null && mMotionLayer.ActiveMotion is SlopSlide_v2)
		  return true;
	 ...
	 public override bool TestUpdate()
	 {
		...
		if (mActorController.Velocity.y < 0 && mMotionController.IsGrounded)
		  return false;
	 ...
	 public override bool Activate(MotionControllerMotion rPrevMotion)
	 {
		...
		m_SlopeSlider = rPrevMotion as SlopSlide;
		...
		Vector3 mLaunchForward = mActorController._Transform.forward;
		mLaunchVelocity = mActorController.Velocity;
		mLaunchVelocity.y = 0;
		//mActorController.SetVelocity(mLaunchVelocity);
		//mLaunchVelocity += Vector3.up * 4;
		//mLaunchVelocity = mActorController.Velocity;
		mActorController.AddImpulse(mActorController._Transform.up * _Impulse);

		mIsImpulseApplied = true;
		//mLaunchVelocity = m_SlopeSlider.moveVector;

		//mLaunchVelocity = rPrevMotion.Movement;
		//Debug.Log(rPrevMotion.Movement);
	 ...
	 {
		TRANS_AnyState_RunningJump = mMotionController.AddAnimatorName("AnyState -> Base Layer.SlopSlideJump-SM.RunningJump");
		TRANS_EntryState_RunningJump = mMotionController.AddAnimatorName("Entry -> Base Layer.SlopSlideJump-SM.RunningJump");
		STATE_Start = mMotionController.AddAnimatorName("Base Layer.Start");
		STATE_IdlePose = mMotionController.AddAnimatorName("Base Layer.SlopSlideJump-SM.IdlePose");
		STATE_RunJump_RunForward = mMotionController.AddAnimatorName("Base Layer.SlopSlideJump-SM.RunJump_RunForward");
		STATE_RunningJump = mMotionController.AddAnimatorName("Base Layer.SlopSlideJump-SM.RunningJump");
		TRANS_RunningJump_RunJump_RunForward = mMotionController.AddAnimatorName("Base Layer.SlopSlideJump-SM.RunningJump -> Base Layer.SlopSlideJump-SM.RunJump_RunForward");
		TRANS_RunningJump_LandToIdle = mMotionController.AddAnimatorName("Base Layer.SlopSlideJump-SM.RunningJump -> Base Layer.SlopSlideJump-SM.LandToIdle");
		STATE_LandToIdle = mMotionController.AddAnimatorName("Base Layer.SlopSlideJump-SM.LandToIdle");
		TRANS_LandToIdle_IdlePose = mMotionController.AddAnimatorName("Base Layer.SlopSlideJump-SM.LandToIdle -> Base Layer.SlopSlideJump-SM.IdlePose");
	 }

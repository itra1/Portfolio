...
namespace it.Game.PlayMaker.AStar
{
  ...
  public abstract class AIBase : VersionedMonoBehaviour
  {
	 ...
	 {
		get { return height.Value * 0.5f; }
		set { height.Value = value * 2; }
	 }
	 ...
	 public Transform target
	 {
		get
		{
		  var setter = _go.GetComponent<AIDestinationSetter>();
		...
		set
		{
		  ...
		  var setter = _go.GetComponent<AIDestinationSetter>();
		  if (setter == null) setter = _go.AddComponent<AIDestinationSetter>();
		  setter.target = value;
	 ...
	 /// </summary>
	 public Vector3 desiredVelocity { get { return lastDeltaTime > 0.00001f ? movementPlane.ToWorld(lastDeltaPosition / lastDeltaTime, 0) : Vector3.zero; } }
	 ...
	 protected virtual bool shouldRecalculatePath
	 {
		get
		{
		  return Time.time - lastRepath >= repathRate.Value && !waitingForPathCalculation && canSearch.Value && !float.IsPositiveInfinity(destination.x);
		}
	 }
	 ...
	 {
		tr = _go.transform;
		seeker = _go.GetComponent<Seeker>();
		rigid = _go.GetComponent<Rigidbody>();
	 }
	 ...
	 protected virtual void OnDrawGizmos()
	 {
		...
		var color = ShapeGizmoColor;
		  ...
		  Draw.Gizmos.Cylinder(position, rotation * Vector3.up, tr.localScale.y * height.Value, radius.Value * tr.localScale.x, color);
	 ...
	 void ResetShape()
	 {
		...
		{
		  radius = cc.radius;
		  height = Mathf.Max(radius.Value * 2, cc.height);
		}
	 }

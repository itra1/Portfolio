...
namespace it.Game.PlayMaker.ActorController
{
  ...
  [HutongGames.PlayMaker.Tooltip("Актор контроллер. Перемещение на позицию")]
  public abstract class MovePosition : ActorDriver
  {
	 ...
	 public bool VerticalRotate = false;
	 ...
	 protected virtual void CalculateMove(Vector3 rWaypoint, ref Vector3 rMove, ref Quaternion rRotate)
	 {
		...
		if (VerticalRotate)
		{
		  Vector3 lleftDirection = Vector3.Project(lDirection, -_go.transform.right);
		  Vector3 lLateralSideDirection = lDirection - lleftDirection;
		  float lXawAngle = Vector3Ext.SignedAngle(_go.transform.forward, lLateralSideDirection);
		  if (rotationSpeed.Value == 0f)
		  {
			 rRotate *= Quaternion.AngleAxis(lXawAngle, -_go.transform.right);
		  }
		  else
		  {
			 rRotate *= Quaternion.AngleAxis(Mathf.Sign(lXawAngle) * Mathf.Min(Mathf.Abs(lXawAngle), rotationSpeed.Value * lDeltaTime), -_go.transform.right);
		  }
		}

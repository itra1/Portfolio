...
{
  public class EnergyProgress : MonoBehaviourBase
  {
	 [SerializeField]
	 ...
	 private float PectItem {	get => 1f / _items.Length; }
	 ...
	 {
		SetValue(true);
		it.Game.Events.EventDispatcher.AddListener(it.Game.Managers.EnergyManager.EVT_ENERGY_CHANGE, HealthChecnge);
		StartCoroutine(CoroutineChange());
	 }

	 private void OnDisable()
	 {
		it.Game.Events.EventDispatcher.RemoveListener(it.Game.Managers.EnergyManager.EVT_ENERGY_CHANGE, HealthChecnge);
		StopAllCoroutines();
	 }
	 ...
	 {
		//_targetValue = GameManager.Instance.EnergyManager.Percent;
		_targetValue = (int)Mathf.Ceil(GameManager.Instance.EnergyManager.Percent / PectItem);
		if (force)
		{
	 ...
	 //  _line.sizeDelta = new Vector2(_minWidth + ((_maxWidth - _maxWidth) * GameManager.Instance.EnergyManager.Percent), _line.sizeDelta.y);
	 //  _currentValue = _targetValue;
	 //}
	 //}

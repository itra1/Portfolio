    ...
    public static class RaycastExt
    {
        ...
        public static bool SafeSpiralCast(Transform rRootTransform, out RaycastHit rHitInfo, float rRadius = 8f, float rDistance = 1000f, float rDegreesPerStep = 27f, int rLayerMask = -1, string rTag = null, Transform rIgnore = null, List<Transform> rIgnoreList = null, bool rIgnoreTriggers = true, bool rDebug = false)
        {
            ...
            for (lCount = 0; lCount < lSteps; lCount++)
            {
                ...
                if (rDebug)
                {
			 com.ootii.Graphics.GraphicsManager.DrawLine(rRootTransform.position, rRootTransform.TransformPoint(lPosition), (lCount == 0 ? Color.red : lColor));
                }
        ...
        public static bool SafeCircularCast(Vector3 rRayStart, Vector3 rRayDirection, Vector3 rRayUp, out RaycastHit rHitInfo, float rDistance = 1000f, float rDegreesPerStep = 30f, int rLayerMask = -1, string rTag = null, Transform rIgnore = null, List<Transform> rIgnoreList = null, bool rIgnoreTriggers = true, bool rDebug = false)
        {
            for (float lAngle = 0f; lAngle <= 360f; lAngle += rDegreesPerStep)
            {
                ...
                if (rDebug)
                {
			 com.ootii.Graphics.GraphicsManager.DrawLine(rRayStart, rRayStart + (lDirection * rDistance), Color.cyan, null, 5f);
                }
	 ...
	 /// <param name="rEdgeHitInfo"></param>
	 /// <returns></returns>
	 public static bool GetForwardEdgeMulty(Transform rTransform, float rMaxDistance, float rMaxHeight, float rMinHeight, int rCollisionLayers, out RaycastHit rEdgeHitInfo, float distanceRepeat = 0.005f)
	 {
		rEdgeHitInfo = RaycastExt.EmptyHitInfo;

		// ѕроверка на максимальную высоту
		Vector3 lRayStart = rTransform.position + (rTransform.up * (rMaxHeight + 0.001f));
		Vector3 lRayDirection = rTransform.forward;
		...
		// стрет€ем сверху, определ€ем точку
		lRayStart = lRayStart + (rTransform.forward * rMaxDistance);
		lRayDirection = -rTransform.up;
		lRayDistance = rMaxHeight;
      lRayDistance = Math.Abs(rMaxHeight - rMinHeight);
      ...
      RaycastHit rEdgeHitInfoTmp = rEdgeHitInfo;
		float tDist = rMaxDistance;
		Vector3 tStart = lRayStart;
      do
		{
		  tDist -= distanceRepeat;
		  tStart = lRayStart + (rTransform.forward * tDist);
		  rEdgeHitInfo = rEdgeHitInfoTmp;
		}
		while (SafeRaycast(tStart, lRayDirection, out rEdgeHitInfoTmp, lRayDistance, rCollisionLayers, rTransform));

		// This is the height of our edge
		float lEdgeHeight = (rMaxHeight + 0.001f) - rEdgeHitInfo.distance;

		// —трел€ем вперед и провер€ем край
		lRayStart = rTransform.position + (rTransform.up * (lEdgeHeight - 0.001f));
		lRayDirection = rTransform.forward;
		lRayDistance = rMaxDistance;
      ...
      lRayStart += rTransform.up * 0.4f;
      float currentDistance = (lRayStart - rEdgeHitInfo.point).magnitude;
      float targetDistance = currentDistance + 0.7f;
      RaycastHit rEdgeHitInfo2;
      if (SafeRaycast(lRayStart, lRayDirection, out rEdgeHitInfo2, targetDistance, rCollisionLayers, rTransform))
      {
        return false;
      }

      // ѕровер€ем наличие места доступного дл€ персонажа на краю

...
/// Биллинг контроллер
/// </summary>
public class BillingManager : Singleton<BillingManager>, IStoreListener {

	public List<BillingProductAbstract> billingProductList;
	...
	public List<BillingProductAbstract> GetBillingProduct(IapType type) {
		return billingProductList.FindAll(x => x.type == type);
	}
	...
	public void ByeProduct(string productId) {
		...
		if (!NetManager.Instance.internetAwalable) {
			...
			noInternet.OnCancel = () => { };
			noInternet.OnRepeat = () => {
				ByeProduct(productId);
			};

			return;
		}

		var prod = billingProductList.Find(x => x.productId == productId);
		if (prod != null)
			ByeProduct(prod);
	}
	...
	public void ByeLocation(int locationNum, Action Callback = null) {
		...
		if (!NetManager.Instance.internetAwalable) {
			...
			noInternet.OnCancel = () => { };
			noInternet.OnRepeat = () => {
				ByeLocation(locationNum, Callback);
			};

			return;
		}

		var locProduct = GetLocationProduct(locationNum);

		if (locProduct == null)
			return;

		ByeProduct(locProduct, Callback);

	}

	public LocationProduct GetLocationProduct(int locationNum) {
		List<BillingProductAbstract> locationProducts = GetBillingProduct(IapType.location);

		for (int i = 0; i < locationProducts.Count; i++) {
			if ((locationProducts[i] as LocationProduct).locationNum == locationNum) {
				return locationProducts[i] as LocationProduct;
			}
		}
		return null;
	}
	...
	public void ByeProduct(BillingProductAbstract product, Action Callback = null) {
		...
		if (!IsInitialized() && !NetManager.Instance.internetAwalable) {
	...
	public void InitializePurchasing() {
		...
		billingProductList.ForEach((prod) => {
	...
	public string GetTransactionToken(int locationNum) {
		try {
			...
			var loca = GetLocationProduct(locationNum);

			if (loca.product.hasReceipt) {

				byte[] toBytes = Encoding.UTF8.GetBytes(ParceReceipt(loca.product.receipt));
				return System.Convert.ToBase64String(toBytes);
				//return loca.product.transactionID;
			}
...
//#endif
		...
		if (Application.platform == RuntimePlatform.IPhonePlayer ||
			...
			//PlayerManager.Instance.company.firstLoad = false;
			apple.RestoreTransactions((result) => {

				if (result) {

					PlayerManager.Instance.company.billingRestore = true;
					billingProductList.ForEach((elem) => {
			...
			//PlayerManager.Instance.company.firstLoad = true;

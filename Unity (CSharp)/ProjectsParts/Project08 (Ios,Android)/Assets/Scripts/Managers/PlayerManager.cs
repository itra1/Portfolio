...
public class PlayerManager : Singleton<PlayerManager> {
	...
	public void Save(bool full = false) {
		...
		saveData.Add("coins", coins.ToString());
		...
		saveData.Add("fish", JsonConvert.SerializeObject(FishManager.Instance.Save()));
		...
		if (full) {
			...
			PlayerManager.Instance.company.SaveAllLocations();
		}
	...
	private void Load(Action OnComplete) {
		try {
			...
			if (saveData.ContainsKey("coins"))
				coins = int.Parse(saveData["coins"]);
			...
			if (saveData.ContainsKey("fish"))
				FishManager.Instance.Load(JsonConvert.DeserializeObject<FishSave>(saveData["fish"]));
			...
			lastEnter = saveData.ContainsKey("lastEnter") ? DateTime.Parse(saveData["lastEnter"]) : DateTime.Now;
			
			if ((DateTime.Now - lastEnter).Days > 3)
				company.CheckAdd3LocationAfter3DayOut();
	...
	public void HintFirstLetter() {
		...
		if (!unlimitedHint && hintFirstLetter <= 0) {
			if (coins <= Properties.Instance.hintFirstLetterPrice) {
				ShopUi shop = (ShopUi)UIManager.Instance.GetPanel(UiType.shop);
				shop.gameObject.SetActive(true);
				shop.SetType(IapType.hintFirstLetter);
				return;
			};
	...
	public void HintAnyLetter() {
		...
		if (!unlimitedHint && hintAnyLetter <= 0) {
			if (coins <= Properties.Instance.hintAnyLetterPrice) {
				ShopUi shop = (ShopUi)UIManager.Instance.GetPanel(UiType.shop);
				shop.gameObject.SetActive(true);
				shop.SetType(IapType.hintAnyLetter);
				return;
			};
	...
	public void HintFirstWord() {
		...
		if (!unlimitedHint && hintFirstWord <= 0) {
			if (coins <= Properties.Instance.hintFirstWordPrice) {
				ShopUi shop = (ShopUi)UIManager.Instance.GetPanel(UiType.shop);
				shop.gameObject.SetActive(true);
				shop.SetType(IapType.hintFirstWord);
				return;
			}; ;

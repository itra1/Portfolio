...
public class LocationsGamePlay : UiPanel {
	...
	protected override void OnEnable() {
		...
		_couchPanel = UIManager.Instance.GetPanel(UiType.conch);
	...
	public void PlayButtonUp(bool isFreePlay = false) {
		...
		if (!_swipe) {
			...
			if (isFreePlay) {
				...
				//var loc = PlayerManager.Instance.company.GetSaveLocation(PlayerManager.Instance.company.actualCompany,
				//	_locationWord.GetFocusIsland().num);
				//if (loc.levels.Count == 0) {
				//	PlayerManager.Instance.company.freePlayLocation = _locationWord.GetFocusIsland().num;
				//	loc.isFreePlay = true;
				...
				//	_locationWord.SelectFocusIsland();
				//}
				var loc = PlayerManager.Instance.company.GetSaveLocation(PlayerManager.Instance.company.actualCompany, _locationWord.GetFocusIsland().num);
				loc.isFreePlay = true;
				_locationWord.SelectFocusIsland();
			}
			else {
	...
	public void ChangeLocation() {
		...
		if (!island.isShare)
			title.SetText(company.locations[island.num].title, LanguageManager.Instance.activeLanuage.code);
		else
			title.SetText(island.location.title, LanguageManager.Instance.activeLanuage.code);

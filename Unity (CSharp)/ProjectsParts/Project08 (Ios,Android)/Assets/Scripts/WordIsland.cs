...
public class WordIsland : MonoBehaviour {
	...
	private List<LetterIsland> prefabListLetter = new List<LetterIsland>();
	public List<LetterIsland> listLetter = new List<LetterIsland>();
	...
	public void SetWord(GameCompany.Word word) {
		...
		GameCompany.Save.Level _saveLevel = PlayerManager.Instance.company.GetSaveLevel(PlayerManager.Instance.company.actualCompany, PlayerManager.Instance.company.actualLocationNum, PlayerManager.Instance.company.actualLevelNum);
	...
	private LetterIsland GetInstance() {

		LetterIsland li = prefabListLetter.Find(x => !x.gameObject.activeInHierarchy);

		if (li == null) {
			GameObject inst = Instantiate(letterPrefab);
			inst.transform.SetParent(transform);
			li = inst.GetComponent<LetterIsland>();

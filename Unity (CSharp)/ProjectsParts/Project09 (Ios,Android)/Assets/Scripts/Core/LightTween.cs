#define SPINE                       // Использование Spine
...
public class LightTween : MonoBehaviour {
  ...
  private void UpdateColor() {
    switch(ct) {
      ...
      case ColorType.sprite:
        ...
        break;
      case ColorType.image:
        ((Image)component).color = color4s[2];
        break;
      case ColorType.text:
        ((Text)component).color = color4s[2];
        break;
#if SPINE
  ...
  public static void SpriteColorTo(Component obj, Color to, float time, float delay, LightTween.EaseType et, GameObject evnt, DelegateOnComplete onComplete) {
    ...
    if(obj.GetType() == typeof(SpriteRenderer)) {
      ...
      lt.color4s[0] = ((Image)obj).color;
      lt.ct = ColorType.image;
    } else if(obj.GetType() == typeof(Text)) {
      lt.color4s[0] = ((Text)obj).color;
      lt.ct = ColorType.text;
    }
    #if SPINE
  ...
  void GenerateColorToTargets() {
    ...
    switch(ct) {
      ...
      case ColorType.text:
        ...
        color4s[0] = color4s[1] = (component as Text).color;
        break;
      case ColorType.image:
        component = GetComponent<Image>();
        color4s[0] = color4s[1] = (component as Image).color;
        break;
#if SPINE
      case ColorType.spine:

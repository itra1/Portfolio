...
/// </summary>
public class GrenadeFireWeaponManager : WeaponManager {
  ...
  protected override void CreateBullet(ObjectSpawn bulletData) {
    ...
    inst.GetComponent<Bullet>().Init(move);
		inst.GetComponent<GrenadeFireBullet>().damageRadius = damageRadius;
		inst.GetComponent<GrenadeFireBullet>().tagetPosition = MapManager.PositionInvers(((ObjectSpawn.BulletData)bulletData.data).targetPoint);
		inst.SetActive(true);

...
[Serializable]
public class BarrierLevel: BarrierSpawner {

  List<float> cd = new List<float>();
  public List<BarrierGenerateParam> barrierParam;                     // Параметры генерации
  private float barrierLastDistanceGroup;           // Последняя дистанция блока
  private float barrierNextDistanceGroup;           // Новое окончание дистанции
  private int barrierCountInGroup;
  private int barrierCountExists;
  private float barrierDistanceInGroup;
  private float fastGenerateDistance;

  barrierGenerateTypes nextType;                                      // Следуюй тип генерации
  public FloatSpan barrierDistanceGenerate;                           // Дистанции генерации преград
  private float nextGenerate;                                                 // Следующая генерация
  private bool barrierGenerateReady;
  private float nextProbility;
  private int ghostCount;
  private bool groupGenerate;
  public List<GroupPrefabs> barrierPref;                              // Набор объектов для пулинга
  public GameObject barriersFolder;                                   // Родитель для создаваемых объектов
  ...
  public override void CalcNextGenerate() {
    ...
    if (barrierNextDistanceGroup <= RunnerController.playerDistantion) {
      ...
      for (int i = 0; i < barrierParam.Count; i++) {
        if ((i > 0 && barrierParam[i].distance > RunnerController.playerDistantion && barrierParam[i - 1].distance < RunnerController.playerDistantion)
          || (i == 0 && barrierParam[i].distance > RunnerController.playerDistantion)) {
    ...
    if (barrierCountExists <= 0) return;
  ...
  public void GetConfig() {
    List<Configuration.Levels.Barriers> itemsData = Config.Instance.config.activeLevel.barriers;
    ...
    for (int i = 0; i < itemsData.Count; i++) {
      ...
      oneBarrier.distance = itemsData[i].distantion;
      oneBarrier.count = itemsData[i].barrier;

...
[Serializable]
public class SpecialPlatformSurvival : SpecialPlatforms {
	...
	void HandingPlatformCalcDist() {
		...
		HandingPlatformParametrs activeParam = handingPlatformParametrs.Find(x => x.distance == calcNext);

		if (activeParam.types.Count > 0) {
			activeHandPlatform = activeParam.GetPlatformType();
			isGenerateReady = true;
	...
	protected override void GetConfig() {
		...
		List<Configuration.Survivles.Platform> itemsData = Config.Instance.config.survival.platforms;
